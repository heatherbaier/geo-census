...
def get_dists(mu, std, n):
    return torch.cat([torch.distributions.Normal(mu, std).rsample() for i in range(n)]).cpu().detach().numpy().tolist()
...
#             print("NEW SIZE: ", size)
    ...
    def denormalize(self, dims, coords):
...
#         print("phi_out.shape: ", phi_out.shape)
