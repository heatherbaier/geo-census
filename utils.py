...
def calc_reward(true, pred):
    ...
    quants = [true * .10, true * .20, true * .30]
...
def get_png_names(directory):
    ...
    for i in os.listdir(directory):
        try:
            new_path = os.path.join(directory, i)
...
def get_y(image_names, mig_data):
    y_mig = []
    for i in image_names:
        dta = mig_data[mig_data["muni_id"] == i.split("/")[3].split("_")[0].replace(".png", "")]
        if len(dta) != 0:
            y_mig.append(dta['var'].values[0])
    return y_mig